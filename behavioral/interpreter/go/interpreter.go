...
// 场景：电商促销规则引擎
//
// 问题：运营人员需要频繁配置不同促销规则，例如：
//   - "vip == true AND order_count > 5"  → 双十一 VIP 专属折扣
//   - "order_count <= 1"                  → 新客免运费
//   - "level > 3 OR spend > 1000"         → 高消费/高等级用户奖励
//
// 如果用 if/else 硬编码，每次新增规则都要修改代码重新上线。
// 解释器模式将规则文本解析为表达式树，运行时直接对用户数据求值，
// 规则新增只需改数据库，无需改代码。
